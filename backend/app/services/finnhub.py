...
def get_etf_profile(symbol: str) -> dict | None:
    """Fetch ETF profile from Finnhub (category, asset class, issuer, etc.).

    Returns a dict with keys like ``assetClass``, ``category``, ``issuer``,
    ``expenseRatio``, ``aum``, etc.  Returns ``None`` on any error, missing
    key, or empty response.
    """
    if not settings.FINNHUB_API_KEY:
        logger.debug("FINNHUB_API_KEY not set – skipping ETF profile for %s", symbol)
        return None

    url = f"{_FINNHUB_BASE}/etf/profile"
    params = {"symbol": symbol, "token": settings.FINNHUB_API_KEY}
    delays = [1, 2, 4]
    for attempt, delay in enumerate(delays):
        try:
            resp = httpx.get(url, params=params, timeout=10.0)
            if resp.status_code == 429:
                logger.warning("Finnhub rate limit hit for ETF profile %s, attempt %d", symbol, attempt + 1)
                time.sleep(delay)
                continue
            resp.raise_for_status()
            data = resp.json()
            ...
            logger.warning("Finnhub ETF profile HTTP error for %s: %s", symbol, exc)
            return None
        except Exception as exc:
            logger.warning("Finnhub ETF profile request error for %s (attempt %d): %s", symbol, attempt + 1, exc)
            if attempt < len(delays) - 1:
                time.sleep(delay)
                continue
            return None
...
def get_etf_sector_exposure(symbol: str) -> list[dict] | None:
    """Fetch ETF sector exposure from Finnhub.

    Returns a list of dicts like ``[{"industry": "Technology", "exposure": 35.2}, ...]``.
    Returns ``None`` on any error, missing key, or empty response.
    """
    if not settings.FINNHUB_API_KEY:
        logger.debug("FINNHUB_API_KEY not set – skipping ETF sector exposure for %s", symbol)
        return None

    url = f"{_FINNHUB_BASE}/etf/sector"
    params = {"symbol": symbol, "token": settings.FINNHUB_API_KEY}
    delays = [1, 2, 4]
    for attempt, delay in enumerate(delays):
        try:
            resp = httpx.get(url, params=params, timeout=10.0)
            if resp.status_code == 429:
                logger.warning("Finnhub rate limit hit for ETF sector %s, attempt %d", symbol, attempt + 1)
                time.sleep(delay)
                continue
            resp.raise_for_status()
            data = resp.json()
            ...
            logger.warning("Finnhub ETF sector HTTP error for %s: %s", symbol, exc)
            return None
        except Exception as exc:
            logger.warning("Finnhub ETF sector request error for %s (attempt %d): %s", symbol, attempt + 1, exc)
            if attempt < len(delays) - 1:
                time.sleep(delay)
                continue
            return None
